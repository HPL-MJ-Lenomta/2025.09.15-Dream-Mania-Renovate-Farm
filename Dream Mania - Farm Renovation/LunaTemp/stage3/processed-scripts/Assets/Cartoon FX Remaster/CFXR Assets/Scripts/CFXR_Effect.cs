...
namespace CartoonFX
{
	...
	public partial class CFXR_Effect : MonoBehaviour
	{
...
#if !DISABLE_CAMERA_SHAKE
			if (cameraShake != null && cameraShake.camEnabled)
			{
...
#if !DISABLE_CAMERA_SHAKE
			if (cameraShake != null && cameraShake.camEnabled && !GlobalDisableCameraShake)
			{
#if UNITY_EDITOR
		...
		void onEditorUpdate()
		{
			...
			{
#if !DISABLE_CAMERA_SHAKE
				if (cameraShake != null && cameraShake.camEnabled && parentParticle.time < particleTime && parentParticle.time < 0.05f)
				{
